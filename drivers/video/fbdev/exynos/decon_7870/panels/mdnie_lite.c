		...
		unsigned long event, void *data)
{
	...
	if (dpui == NULL) {
		dev_err(mdnie->dev, "%s: dpui is null\n", __func__);
		return 0;
	}
	...
	size = scnprintf(tbuf, MAX_DPUI_VAL_LEN, "%d %d",
			mdnie->coordinate[0], mdnie->coordinate[1]);
	set_dpui_field(DPUI_KEY_WCRD, tbuf, size);

	size = scnprintf(tbuf, MAX_DPUI_VAL_LEN, "%d %d %d", mdnie->white_balance_r,
			mdnie->white_balance_g, mdnie->white_balance_b);
	set_dpui_field(DPUI_KEY_WOFS, tbuf, size);
