...
static ssize_t st_lsm6ds3_accel_dhr_sensor_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 fs_reg_val = 0;
	u8 fs = 0;
	...
	st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_CTRL1_ADDR, 1, &fs_reg_val);
	fs_reg_val &= ST_LSM6DS3_ACCEL_FS_MASK;
	fs_reg_val = fs_reg_val >> 2;

	switch (fs_reg_val) {
	case ST_LSM6DS3_ACCEL_FS_2G_VAL:
		fs = 2;
		break;
	case ST_LSM6DS3_ACCEL_FS_4G_VAL:
		fs = 4;
		break;
	case ST_LSM6DS3_ACCEL_FS_8G_VAL:
		fs = 8;
		break;
	case ST_LSM6DS3_ACCEL_FS_16G_VAL:
		fs = 16;
		break;
	default:
		fs = 4;
...
static DEVICE_ATTR(dhr_sensor_info, S_IRUSR | S_IRGRP,
				st_lsm6ds3_accel_dhr_sensor_info_show, NULL);
