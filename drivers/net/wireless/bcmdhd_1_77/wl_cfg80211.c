 ...
 *
 * $Id: wl_cfg80211.c 685344 2017-02-16 09:23:37Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
#endif
{
	...
	struct ether_addr addr_null = {{0, 0, 0, 0, 0, 0}};
	RETURN_EIO_IF_NOT_UP(cfg);
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		...
		if (curmacp != NULL && !memcmp(curmacp, &addr_null, ETHER_ADDR_LEN)) {
			if (fw_assoc_state) {
				WL_ERR(("dongle is associated, but profile bssid is null"));
				fw_assoc_state = FALSE;
			}
		}
		if (!dhd_assoc_state || !fw_assoc_state) {
			...
			if (err == -ENODATA)
				return err;
			WL_TRACE_HW4(("drv state : %d, fw state : %d\n",
				dhd_assoc_state, fw_assoc_state));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	/* Reset Profile BSSID and Last Roam Address
	 * to enable roam event to supplcicant everytime
	 * a roam event comes from fw.
	 */
	wl_update_prof(cfg, ndev, NULL, NULL, WL_PROF_BSSID);
