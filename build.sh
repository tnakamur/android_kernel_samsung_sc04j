...
# ---------
BUILD_SCRIPT=2.34
VERSION_NUMBER=$(<ramdisk/version)
ARCH=arm64
...
DTB_PADDING=0
KERNELNAME=TNKernel
KERNELCONFIG=exynos7870-a3xpremiumlte_defconfig
...
if [ -n `which java` ]; then
	...
	java -Xmx1024m -jar $RDIR/ramdisk/signapk/signapk.jar -w $RDIR/ramdisk/signapk/testkey.x509.pem $RDIR/ramdisk/signapk/testkey.pk8 unsigned-$ZIP_NAME $ZIP_NAME
...
mv -f $RDIR/ramdisk/image-new.img $RDIR/boot.img
...
ZIP_NAME=TNKernel_SC04J_v$VERSION_NUMBER.zip
...
rm -rf ./build.log
clear
echo "TNKernel Build Script v$BUILD_SCRIPT -- Kernel Version: v$VERSION_NUMBER"
echo ""
